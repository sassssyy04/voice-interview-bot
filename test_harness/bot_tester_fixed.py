"""Automated test harness for voice bot evaluation - Fixed version."""
...
class BotTester:
    ...
    async def run_single_test(self, utterance: Dict[str, Any]) -> TestResult:
        ...
        # Start a new conversation session for this test
        ...
        logger.info(f"✓ Test completed: {utterance['id']}")
        logger.info(f"  Expected: {utterance['transcript'][:60]}...")
        logger.info(f"  Actual:   {actual_transcript[:60]}...")
        logger.info(f"  Similarity: {transcript_similarity:.3f}")
        ...
        return TestResult(
            ...
            turn_count=1,
            completion_status="completed" if actual_transcript else "failed",
    ...
    def calculate_aggregate_metrics(self, test_results: List[TestResult]) -> Dict[str, float]:
        ...
        # Success rate (based on transcript similarity)
        ...
        return {
            "overall_entity_f1": 0.0,  # Not implemented yet
            "per_slot_f1": {},
    ...
    async def run_test_suite(self, utterance_ids: Optional[List[str]] = None) -> TestSession:
        ...
        for i, utterance in enumerate(utterances, 1):
            try:
                logger.info(f"\n--- Test {i}/{len(utterances)}: {utterance['id']} ---")
                result = await self.run_single_test(utterance)
    ...
    def print_metrics_report(self, test_session: TestSession):
        ...
        for result in test_session.test_results:
            status = "✓" if result.completion_status == "completed" else "✗"
            similarity = f"{result.transcript_similarity:.3f}" if result.transcript_similarity > 0 else "N/A"
            print(f"  {status} {result.utterance_id:12} | Similarity: {similarity:5} | {result.actual_transcript[:40]}...")
